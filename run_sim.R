...
run_sim <- function(sim_fn,setting_param_list, other_param_list,nworkers = 30){
  
  
  future::plan("multiprocess",workers =  min(nworkers,detectCores()-1))
  ...
  results <- results %>%
    ...
    dplyr::mutate(res = 
                    furrr::future_map(data,
                                      ~sim_fn(., other_param_list=other_param_list)
                                      ,      .progress = TRUE
                    )) %>%
    tidyr::unnest(cols = "data") 
  results <- mutate(results,res = purrr::map(res,function(resi){return(resi$result)})) %>% 
